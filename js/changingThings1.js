...
(function(){
	...
	var workImages = document.querySelectorAll('.portfolioCon'),
		workSub = document.querySelector('.imgCaption h3'),
		workText = document.querySelector('.imgCaption p'),
		appliedClass; // set our globals
	
	function changeWork() {	
		
		workSub.classList.remove(appliedClass); 
		
		appliedClass = this.id; 
		
		workSub.classList.add(this.id); 
		
		workSub.firstChild.nodeValue = portfolioWork[this.id].headline;
		workText.firstChild.nodeValue = portfolioWork[this.id].text; 
	}

	[].forEach.call(workImages, function(image) {
		image.addEventListener("click", changeWork, true);
	});

	workSub.firstChild.nodeValue = portfolioWork['portfolio1'].headline; 
	workText.firstChild.nodeValue = portfolioWork['portfolio1'].text; 
})();
